...
if ($Start -eq -1)
{
    Write-Debug "Could not get the links, please check the StoreURL."
    exit
}
...
{
    Write-Debug "Invalid characters in path"$SavePathRoot\$ProductID""
    exit
}
...
{
    Write-Debug "Creating Directorty"$SavePathRoot\$ProductID""
    ...
    {
        Write-Debug "Failed to create directory.$([System.environment]::NewLine)$_"
        exit
    }
}
...
{
    Write-Debug "Downloading $($Download.textContent)..."
