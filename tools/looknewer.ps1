...
Install-PackageProvider -name winget
    ...
    if($Todo.Count -eq 0 -and $issue -eq 0) {
        ...
        foreach ($issue in $issues) {
            ...
            "Checking $search"
            if(!((Get-GitHubIssue -OwnerName $Owner -RepositoryName $Repository | Where-Object {$_.title -match "($search)"} | Where-Object {$_.created -lt $((Get-Date).AddDays(-90))})) -and (((Get-Content .\Check\exclude.txt | Select-String -Pattern $search).Matches.Success))) {
