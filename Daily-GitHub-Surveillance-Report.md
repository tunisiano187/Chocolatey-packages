## 📋 Rapport de Surveillance Quotidienne GitHub

**Date**: 2026-06-21 08:10:21 UTC  
**Repository**: tunisiano187/Chocolatey-packages  
**Période**: 1 jour(s)  

---

### 🚀 Issues Ouverts (29 total)

#### 🔴 critical (5 issues)
- #4084: 🚨 Surveillance Quotidienne - Critiques 2026-06-20 - https://github.com/tunisiano187/Chocolatey-packages/issues/4084 [🔴 critical]
- #4082: fix(chromehistoryview): sync nuspec to v1.53 to stop 409 push failures (CHO-487) - https://github.com/tunisiano187/Chocolatey-packages/issues/4082 [🔴 critical]
- #4081: 🚨 Surveillance Quotidienne - Critiques 2026-06-19 - https://github.com/tunisiano187/Chocolatey-packages/issues/4081 [🔴 critical]
- #4069: fix(bonjour): replace em dash with hyphen to fix CP1252 parse error (CHO-471) - https://github.com/tunisiano187/Chocolatey-packages/issues/4069 [🔴 critical]
- #4065: fix(windjview): use Get-FileVersion to avoid SourceForge /download path error (CHO-461) - https://github.com/tunisiano187/Chocolatey-packages/issues/4065 [🔴 critical]

#### 🟠 high (3 issues)
- #4090: fix(jitsi-meet-electron): replace Get-GitHubRelease with direct REST call - https://github.com/tunisiano187/Chocolatey-packages/issues/4090 [🟠 high]
- #4089: fix(install4j): replace choco search with upstream website query - https://github.com/tunisiano187/Chocolatey-packages/issues/4089 [🟠 high]
- #4088: fix(Executor): fix regex to select current release instead of previousdownloads link - https://github.com/tunisiano187/Chocolatey-packages/issues/4088 [🟠 high]

#### ⚪ normal (21 issues)
- #4096: fix(softwareinformer): add null check and PE header validation (CHO-502) - https://github.com/tunisiano187/Chocolatey-packages/issues/4096 [⚪ normal]
- #4095: fix(vnc-connect): add browser User-Agent header to bypass 403 (CHO-504) - https://github.com/tunisiano187/Chocolatey-packages/issues/4095 [⚪ normal]
- #4094: fix(projectlibre.portable): replace Get-RedirectedUrl with SourceForge RSS feed (CHO-501) - https://github.com/tunisiano187/Chocolatey-packages/issues/4094 [⚪ normal]
- #4093: fix(performancetest): avoid 403 by scraping version from history page - https://github.com/tunisiano187/Chocolatey-packages/issues/4093 [⚪ normal]
- #4092: fix(paraview): replace .Links with regex on Content to fix null expression - https://github.com/tunisiano187/Chocolatey-packages/issues/4092 [⚪ normal]
- #4091: fix(osfmount): replace HTML scraping with direct EXE version extraction - https://github.com/tunisiano187/Chocolatey-packages/issues/4091 [⚪ normal]
- #4087: fix(eagle): add Mozilla UserAgent to prevent 403 on eagle-updates.circuits.io - https://github.com/tunisiano187/Chocolatey-packages/issues/4087 [⚪ normal]
- #4086: fix(par2cmdline): hardcode Owner/repo to fix broken URL parsing - https://github.com/tunisiano187/Chocolatey-packages/issues/4086 [⚪ normal]
- #4085: fix(mpc-qt): hardcode Owner/repo to fix broken URL parsing - https://github.com/tunisiano187/Chocolatey-packages/issues/4085 [⚪ normal]
- #4083: fix(cura-lulzbot): scrape software.lulzbot.com instead of broken lulzbot.com - https://github.com/tunisiano187/Chocolatey-packages/issues/4083 [⚪ normal]
- #4080: chore(deps): update actions/checkout action to v7 - https://github.com/tunisiano187/Chocolatey-packages/issues/4080 [⚪ normal]
- #4077: fix(tigervnc): move file downloads from au_GetLatest to au_BeforeUpdate (CHO-485) - https://github.com/tunisiano187/Chocolatey-packages/issues/4077 [⚪ normal]
- #4075: fix(jcpicker): use Regex::Match + trailing-slash URL, normalize version to 3 parts - https://github.com/tunisiano187/Chocolatey-packages/issues/4075 [⚪ normal]
- #4074: fix(captureflux): use specific URL match to avoid fragile zip$-first selection - https://github.com/tunisiano187/Chocolatey-packages/issues/4074 [⚪ normal]
- #4073: fix(pwgen.portable): replace brittle line-split XML parsing with [xml] cast - https://github.com/tunisiano187/Chocolatey-packages/issues/4073 [⚪ normal]
- #4072: fix(pwgen.install): replace brittle line-split XML parsing with [xml] cast - https://github.com/tunisiano187/Chocolatey-packages/issues/4072 [⚪ normal]
- #4070: fix(nteract.install): replace deprecated update-package with update (CHO-466) - https://github.com/tunisiano187/Chocolatey-packages/issues/4070 [⚪ normal]
- #4068: fix(winflector-client): add au_BeforeUpdate to skip Get-RemoteFiles (CHO-462) - https://github.com/tunisiano187/Chocolatey-packages/issues/4068 [⚪ normal]
- #4066: fix(nuclear): replace deprecated update-package with update (CHO-463) - https://github.com/tunisiano187/Chocolatey-packages/issues/4066 [⚪ normal]
- #4064: fix(projectlibre.install): keep SourceForge /download URL, compute checksum via Get-FileVersion - https://github.com/tunisiano187/Chocolatey-packages/issues/4064 [⚪ normal]
- #3784: Dependency Dashboard - https://github.com/tunisiano187/Chocolatey-packages/issues/3784 [⚪ normal]

### 🔄 Pull Requests en Attente (0 total)

Aucun Pull Request détecté

---

### 📊 Résumé par Priorité

- 🔴 critical: 5
- 🟠 high: 3
- ⚪ normal: 21

### 📈 Statistiques Globales
- **Total issues**: 29
- **Total PRs**: 0
- **Critiques**: 5
- **Hautes priorités**: 3

### 🚀 Actions Recommandées
- 🔴 **Critique**: Répondre dans les 24 heures
- 🟠 **Haute**: Traiter dans les 3 jours  
- 🟡 **Moyenne**: À traiter bientôt
- 🔵 **Basse**: Peut attendre
- ⚪ **Normale**: Surveillance standard

---

*Généré automatiquement par Enhanced Surveillance Script à 2026-06-21 08:10:21 UTC*